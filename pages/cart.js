...
export default function Cart() {
	...
	const completeOrder = paymentTypeId => {
		const paymentId = parseInt(paymentTypeId)
		completeCurrentOrder(cart.id, paymentId).then(() =>
	...
	const deleteCartItems = async () => {
		for (const item of cart.lineitems) {
			await deleteCart(item.product.id)
		}
		await router.push("/")
	}
		...
		<>
			...
			<CardLayout title="Your Current Order">
				...
				<>
					...
					<a className="card-footer-item" onClick={deleteCartItems}>
						Delete Order
					</a>
				</>
			</CardLayout>
